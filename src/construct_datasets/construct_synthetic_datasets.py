...
for fam in tqdm(list(phased_fam_dict.keys())):
    ...
    if REPEAT_AVG == 1:
        mom_g1 = (np.random.random(N)<=freqs)
        dad_g1 = (np.random.random(N)<=freqs)
        mom_g2 = (np.random.random(N)<=freqs)
        dad_g2 = (np.random.random(N)<=freqs)
    else:
...
counts.to_csv(OUT_DIR + 'synthetic_data_%i_repeats.tsv' % REPEAT_AVG, sep='\t', header=None, index=None)
